...
class EventCreator extends StatefulWidget {
  ...
  EventCreator([this._event]) {
    if (this._event == null) {
      ...
      _event.start = TimeUtils.truncateDate(_event.start, "day").add(Duration(hours: 6));
...
class EventCreatorState extends State<EventCreator> {
  ...
  void initState() {
    ...
    enabledField = !widget._event.id.isNullOrEmpty()
        ? !widget._event.start.isBefore(now.subtract(Duration(minutes: 4)))
        : true;
    if (!widget._event.documents.isNullOrEmpty()) {
      ...
      return GestureDetector(
          ...
          child: Container(
              ...
              decoration: BoxDecoration(
                  color: (_radioValue == index) ? black : white,
              ...
              child: Row(children: <Widget>[
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                      borderRadius: BorderRadius.all(Radius.circular(5.0)), color: HexColor(_categoriesC[index])),
                ),
                new Text(_categoriesN[index].toUpperCase(),
                    style: (_radioValue == index) ? subtitle_rev : subtitle.copyWith(color: black)),
              ])));
    }).toList();

    List<Widget> listOp = (widget._event.operator != null
            ? [widget._event.operator, ...widget._event.suboperators]
      ...
      return Container(
        ...
        child: Row(
          children: <Widget>[
            ...
            op != widget._event.operator && enabledField
                ? IconButton(
                    icon: Icon(
                      Icons.delete,
                      color: black,
          ...
          body: SingleChildScrollView(
            ...
            child: new Form(
                ...
                child: new Column(children: <Widget>[
                      ...
                      ? Row(
                          children: <Widget>[
                            Expanded(
                              ...
                              child: Column(
                                children: buildAutocompleteMaps(),
                              ),
                            ))
                          ],
                        )
                              ...
                              child: new Scrollbar(
                                ...
                                itemBuilder: (BuildContext context, int index) {
                                  ...
                                  final String name = (isMultiPath
                                      ? 'File: ${index + 1}' + _paths.keys.toList()[index]
                                      : 'File : ' + _fileName ?? '...');
                      ...
                      children: <Widget>[
                        Row(children: <Widget>[
                          ...
                          Expanded(
                            child: DateTimeField(
                              decoration: InputDecoration(
                                  hintText: 'Mon 1 Sep 2019', hintStyle: label, border: InputBorder.none),
                              style: label,
                              format: dateFormat,
                              ...
                              onShowPicker: (context, currentValue) {
                                return showDatePicker(
                                    context: context,
                                    firstDate: TimeUtils.truncateDate(now, "day"),
                                    initialDate: currentValue != null
                                        ? currentValue.year > 2000
                                            ? currentValue
                                            : DateTime(2000 + currentValue.year, currentValue.month, currentValue.day,
                              ...
                              validator: (val) {
                                if (val != null) {
                                  widget._event.start = TimeUtils.truncateDate(val, "day");
                                  return null;
                                } else {
                                  return 'Inserisci una data valida';
                                }
                              },
                              onSaved: (DateTime value) => widget._event.start = TimeUtils.truncateDate(value, "day"),
                            ),
                          ),
                          _allDayFlag
                              ? Container()
                                      ...
                                      validator: (val) {
                                        ...
                                        if (val != null &&
                                            ...
                                            (TimeUtils.truncateDate(widget._event.start, "day") ==
                                                    TimeUtils.truncateDate(now, "day")
                                                ? val.hour > now.hour ||
                                                    (val.hour == now.hour && val.minute > now.minute)
                                                : true)) {
                                          widget._event.start = TimeUtils.truncateDate(widget._event.start, "day")
                                              .add(Duration(hours: val.hour, minutes: val.minute));
                                          return null;
                                        } else {
                                  ...
                                  Expanded(
                                    child: DateTimeField(
                                      ...
                                      onShowPicker: (context, currentValue) {
                                        return showDatePicker(
                                            ...
                                            initialDate: currentValue != null
                                                ? currentValue.year > 2000
                                                    ? currentValue
                                                    : DateTime(2000 + currentValue.year, currentValue.month,
                                                        currentValue.day, currentValue.hour, currentValue.minute)
                                                : TimeUtils.truncateDate(now, "day"),
                                            lastDate: DateTime(3000));
                                      },
                                      onChanged: (newValue) => resetOperatorsFields(),
                                      validator: (val) {
                                        if (_allDayFlag) return null;
                                        if ((val != null) &&
                                            (val.year >= 2015 && val.year <= 3000) &&
                                            widget._event.start.isBefore(val.add(Duration(days: 1)))) {
                                          widget._event.end = TimeUtils.truncateDate(val, "day");
                                          return null;
                                        } else {
                                          return 'Inserisci una data valida';
                                        }
                                      },
                                      onSaved: (DateTime value) => widget._event.end = TimeUtils.truncateDate(value, "day"),
                                    ),
                                  ),
                                  Expanded(
                                    child: new DateTimeField(
                  ...
                  Row(children: <Widget>[
                    ...
                    Expanded(
                      child: Padding(
                          ...
                          child: Text(enabledField ? "Aggiungi operatore" : "Operatori",
                              style: label.copyWith(color: colorValidator))),
                    ),
                    enabledField
                        ? IconButton(
                            ...
                            onPressed: enabledField
                                ? () async {
                                    if (!_formDateKey.currentState.validate())
                                      return PlatformUtils.onErrorMessage("Inserisci un intervallo temporale valido");
                  ...
                  Container(child: Column(children: listOp)),
                  Divider(height: 20, indent: 20, endIndent: 20, thickness: 2, color: grey_light2),
                  Container(
                    child: Row(
                      ...
                      children: <Widget>[
                        ...
                        Expanded(child: Column(children: listCat))
                      ],
                    ),
                  ),
                  Divider(height: 20, indent: 20, endIndent: 20, thickness: 2, color: grey_light2),
  ...
  Future _saveNewEvent(BuildContext context) async {
    if (widget._event.operator == null)
      setState(() {
        colorValidator = red;
      });
    else
      setState(() {
        colorValidator = black;
      });
    if ((this._formDateKey.currentState.validate() || !enabledField) &&
      ...
      try {
        ...
        _paths?.forEach((singleName, singlePath) {
          ...
          if (singlePath.contains("/"))
            PlatformUtils.storagePutFile(widget._event.id + "/" + singleName, PlatformUtils.file(singlePath));
        });
  ...
  List<Widget> buildAutocompleteMaps() {
    List<Widget> listPlace = _placesList != null
        ? _placesList.map((place) {
            return GestureDetector(
