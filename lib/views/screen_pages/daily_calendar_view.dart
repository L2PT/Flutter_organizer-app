...
class _DailyCalendarState extends State<DailyCalendar> with TickerProviderStateMixin {
  ...
  List<Widget> _buildFront() {
    ...
    if (_gridHourSpan == 0) {
      ...
      _selectedEvents.forEach((e) {
        r.add(SizedBox(
            height: ((minDailyHour(e.start) - (base.hour * 60 + base.minute)) / 60) / _gridHourSpan * _gridHourHeight));
        r.add(
            Row(children: <Widget>[
              Expanded(
                  flex: 2,
  ...
  int minDailyHour(DateTime start) {
    return (start.day != _selectedDay.day ? Constants.MIN_WORKTIME * 60 : max<int>(
        Constants.MIN_WORKTIME * 60, start.hour * 60 + start.minute));
  }

  int maxDailyHour(DateTime end) {
    return (end.day != _selectedDay.day ? Constants.MAX_WORKTIME * 60 : min<int>(
